...
//double  word_pruning;
  ...
  for (file_name = feature_files;; file_name = file_name->mpNext) 
  {
    ...
    for(t = 0; t < header.mNSamples; t++) {
      ...
      Mixture* mix = hmmRecords[0].hmm->mpState[0]->mpMixture[0].mpEstimates;
      FLOAT* l_obs = XformPass(mix->mpInputXform, feature_matrix[t], time, FORWARD);
      FLOAT* pstrs = XformPass(mix->mpGaussianSelectionInputXform, feature_matrix[t], time, FORWARD);
      
      for(i = 0; i < hmmRecords.size(); i++) {
        State* pState = hmmRecords[i].hmm->mpState[0];
        FLOAT tmpScore = LOG_0;
        
        for (j = 0; j < pState->mNMixtures; j++) {
          Mixture* mix = pState->mpMixture[j].mpEstimates;
          if(pstrs[mix->mGaussianSelectionIndex-1] > 0.0) {
            FLOAT glike =  Decoder<_DecoderNetwork>::DiagCGaussianDensity(mix, l_obs, NULL);
            tmpScore = LogAdd(tmpScore, glike + pState->mpMixture[j].mWeight);
          }
        }
        hmmRecords[i].score += tmpScore;
      }
    }
