  ...
  for (file_name = feature_files;; file_name = file_name->mpNext) 
  {
    ...
    for(t = 0; t < header.mNSamples; t++) 
    {
      ...
      for (i = 0; i < pState->mNMixtures; i++)
      {
        ...
        for(j = 0; nBestList[j].like <= glike && j < nbest; j++) {}
      ...
      for(j = nbest-1; j >= 0; j--)
        tmpScore = LogAdd(tmpScore, nBestList[j].like);
      
      hmmRecords[0].score  += tmpScore;
      ...
      for(i = 1; i < hmmRecords.size(); i++)
      {
        ...
        for(j = nbest-1; j >= 0; j--)
        {
          ...
          tmpScore = LogAdd(tmpScore, glike + pState->mpMixture[nBestList[j].index].mWeight);
        }
